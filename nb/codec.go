...
// Codec is a coder-decoder of voice data at 8000 Hz.
type Codec interface {
	...
	Encode(dst []byte, src []int16) error
...
func checkEncodeBounds(codec Codec, dst []byte, src []int16) error {
	var (
		srcLen = len(src)
		dstLen = len(dst)
...
func checkDecodeBounds(codec Codec, dst []int16, src []byte) error {
	var (
		srcLen = len(src)
		dstLen = len(dst)
